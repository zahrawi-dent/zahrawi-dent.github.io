...
// // SidebarFindpage.jsx
// import { createEffect, For, Show, onMount, onCleanup } from "solid-js";
// import { isServer } from "solid-js/web";
// import BookmarksList from "./BookmarkList";
// import ThemeToggle from "./ThemeToggle";
// import { state, actions } from "../stores/sidebarStore";
//
// // --- Icon Components (keep as they are) ---
// const ChevronDownIcon = () => (
//   <svg class="h-5 w-5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
//   let searchInput;
//
//   // Apply categories from props if provided
//   if (props.categories && props.categories.length > 0) {
//     actions.updateCategories(props.categories);
//   }
//
//   // --- Effects ---
//   createEffect(() => {
//     if (!isServer) {
//       actions.setActiveNavItem(window.location.pathname);
//       actions.initializeTheme(); // Ensure theme is synced on mount
//     }
//   });
//
//   // Effect to handle focusing input and loading Pagefind when search opens
...
//       }
//     } else if (!state.isSearchOpen && state.searchQuery !== "") {
...
//
//       if (pf && typeof pf.search === "function") {
//         actions.setPagefindApi(pf);
//         console.log("Pagefind loaded successfully.");
//         actions.setPagefindLoadError(null);
//
//         // If there's a query pending, perform the search now
//         if (state.searchQuery.trim().length >= MIN_SEARCH_LENGTH) {
//           debouncedSearch(state.searchQuery.trim());
//         }
//       } else {
//         console.error("Pagefind module loaded, but API not found or invalid.");
//         actions.setPagefindLoadError("Pagefind loaded incorrectly. Search unavailable.");
//       }
//     } catch (e) {
//       console.error("Error loading Pagefind:", e);
//       actions.setPagefindLoadError("Failed to load search functionality.");
//     }
//   };
//
//   // Debounced search function using Pagefind
...
//         console.log(`Found ${resultsData.length} results.`);
//         actions.setSearchResults(resultsData);
//         actions.setSelectedIndex(0);
//       }
//     } catch (e) {
//       console.error("Pagefind search error:", e);
...
//     if (query.length >= MIN_SEARCH_LENGTH && state.pagefindApi) {
//       debouncedSearch(query);
...
//       case "ArrowDown":
//         e.preventDefault();
//         actions.setSelectedIndex((state.selectedIndex + 1) % results.length);
//         break;
//       case "ArrowUp":
//         e.preventDefault();
//         const newIndex = state.selectedIndex - 1;
//         actions.setSelectedIndex(newIndex < 0 ? results.length - 1 : newIndex);
//         break;
//       case "Enter":
//         e.preventDefault();
...
//             class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/90 p-4 backdrop-blur-sm"
//             onClick={(e) => {
//               if (e.target === e.currentTarget) actions.toggleSearch();
//             }}
//           >
//             <div class="mx-auto mt-4 max-w-2xl overflow-hidden rounded-lg bg-gray-800 shadow-xl md:mt-16">
//               <div class="relative flex items-center border-b border-gray-700">
...
//                         if (item.children) {
//                           e.preventDefault();
//                           actions.toggleCategory(item.title);
//                         } else {
//                           if (state.isMobileMenuOpen) actions.toggleMobileMenu();
//                         }
//                       }}
...
//           <div class="fixed inset-0 z-20 bg-black/50 lg:hidden" onClick={actions.toggleMobileMenu} aria-hidden="true" />
//         </Show>
//       </div>
//     </>
//   );
