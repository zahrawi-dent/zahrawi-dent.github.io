...
export default function SidebarFindpage(props) {
  ...
  // Update the categories in the store when props change
  createEffect(() => {
    if (props.categories && props.categories.length > 0) {
      store.updateCategories(props.categories);
    }
  });
  ...
  createEffect(() => {
    if (!isServer) {
      store.setActiveNavItem(window.location.pathname);
    }
  });
  ...
  const ensurePagefindLoaded = async () => {
    ...
    try {
      ...
      if (pf && typeof pf.search === "function") {
        store.setPagefindApi(pf);
        console.log("Pagefind loaded successfully.");
        store.setPagefindLoadError(null);
      } else {
        console.error("Pagefind module loaded, but API not found or invalid.");
        store.setPagefindLoadError("Pagefind loaded incorrectly. Search unavailable.");
        store.setPagefindApi(null);
      }
    } catch (e) {
      console.error("Error loading Pagefind:", e);
      store.setPagefindLoadError("Failed to load search functionality.");
      store.setPagefindApi(null);
    }
  };
  ...
  const debouncedSearch = debounce(async (query) => {
    ...
    try {
      ...
      if (search.results.length === 0) {
        ...
        store.setSearchResults(resultsData);
        store.setSelectedIndex(0);
      }
    } catch (e) {
  ...
  const handleKeyDown = (e) => {
    ...
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        store.setSelectedIndex((state.selectedIndex + 1) % results.length);
        break;
      case "ArrowUp":
        e.preventDefault();
        const newIndex = state.selectedIndex - 1;
        store.setSelectedIndex(newIndex < 0 ? results.length - 1 : newIndex);
        break;
      case "Enter":
        e.preventDefault();
      ...
      <div class="flex flex-col pt-16 lg:flex-row lg:pt-0">
        <Show when={state.isSearchOpen}>
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) store.toggleSearch();
            }}
          >
                  ...
                  <li>
                    <a
                      ...
                      onClick={(e) => {
                        if (item.children) {
                          e.preventDefault();
                          store.toggleCategory(item.title);
                        } else {
                          if (state.isMobileMenuOpen) store.toggleMobileMenu();
                        }
                      }}
        ...
        <Show when={state.isMobileMenuOpen}>
          <div class="fixed inset-0 z-20 bg-black/50 lg:hidden" onClick={() => store.toggleMobileMenu()} aria-hidden="true" />
        </Show>
      </div>
    </>
  );
